...
heuristic_schedule_all(Room, Day) :-
    ...
    get_time(Tf),
    T is Tf - Ti,
    write('Escalonamento concluÌdo. Tempo final da ˙ltima operaÁ„o: '), write(TFinal), nl, write('Tempo de execuÁ„o: '), write(T), write('s'),nl.


try_each_permutation(Room, Day, LOpCode) :-
    permutation(LOpCode, Permuted),
    \+attempt_schedule(Room, Day, Permuted), % Falhou? Impede novas tentativas.
    !, % Corta futuras execuÁıes de permutaÁ„o.
...
attempt_schedule(Room, Day, LOpCode) :-
    ...
    agenda_operation_room(Room,Day,Agenda),assert(agenda_operation_room1(Room,Day,Agenda)),
    findall(_,(agenda_staff1(D,Day,L),free_agenda0(L,LFA),adapt_timetable(D,Day,LFA,LFA2),assertz(availability(D,Day,LFA2))),_),
    % Tentar escalonar com a permutaÁ„o atual
...
% Seleciona a prÛxima cirurgia a ser escalonada com base na disponibilidade inicial dos mÈdicos
...
% Encontra a primeira janela disponÌvel suficiente para uma cirurgia
